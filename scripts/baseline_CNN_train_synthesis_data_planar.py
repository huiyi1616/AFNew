...
# 在训练代码中加载 electrode 坐标（仅执行一次）
points = pd.read_csv('triElectrodeFlat_Points.csv', header=None).values  # (19, 2)
points_tensor = torch.tensor(points, dtype=torch.float32)  # 暂时不放到 device 上

def spatial_loss(pred_times, coords, neighbor_radius=5.0):
    """
    综合空间一致性损失：
    - monotonicity_loss：时间应该随空间传播而单调增加
    - directional_loss：时间变化方向应与空间方向一致（dot product 为正）

    返回：
    - monotonicity_loss: 单调性惩罚
    - directional_loss: 方向性惩罚
    """
    B, N = pred_times.shape
    device = pred_times.device
    coords = coords.to(device)

    diffs = coords.unsqueeze(0) - coords.unsqueeze(1)  # (N, N, 2)
    dist_matrix = torch.norm(diffs, dim=-1)
    adj_matrix = (dist_matrix < neighbor_radius).float() - torch.eye(N, device=device)
    ...
    for i in range(N):
        for j in range(N):
            if adj_matrix[i, j] > 0:
                dt = pred_times[:, i] - pred_times[:, j]  # (B,)
                # --- 单调性约束 ---
                loss_mono += torch.mean(F.relu(-dt))  # 如果反向激活，则惩罚

                # --- 方向一致性约束 ---
                spatial_vec = coords[i] - coords[j]  # (2,)
                spatial_unit = F.normalize(spatial_vec.unsqueeze(0), dim=1)  # (1, 2)
                time_vec = spatial_unit * dt.unsqueeze(1)  # (B, 2)
                alignment = torch.sum(time_vec * spatial_unit, dim=1)  # (B,)
                loss_dir += torch.mean(F.relu(-alignment))  # 惩罚方向相反

                count += 1

    loss_mono /= (count + 1e-6)
    loss_dir /= (count + 1e-6)

    return loss_mono, loss_dir
...
def train(model, dataloader, criterion_map, criterion_time, optimizer, device):
    ...
    for electrograms, activation_maps, activation_times_target in tqdm(dataloader, desc="Training"):
        ...
        # loss_mono, loss_dir = spatial_loss(activation_time_pred, points_tensor.to(device))
        #loss = 0.5 * loss_map + 0.3 * loss_time + 0.1 * loss_mono + 0.1 * loss_dir
        #loss = 0.5 * loss_map + 0.3 * loss_time + 0.2 * loss_dir
        loss.backward()
        optimizer.step()
...
# **📌 主训练过程**
def main():
    train_data_dir = r"C:\AF\NickMatlab\Library\RetroMapping_GIT\DataProcessing\DataProcessing\AT_data\20250206\Processed_MAT_Files\dataloader_data\sythesis_data\train"
    ...
    tb_writer  = SummaryWriter(log_dir="runs_AFNet")
    # 初始化 CSV 文件并写入表头
    ...
    for epoch in range(epochs):
        ...
        if val_direction_vector_score < best_val_direction_vector_score:
            ...
            torch.save(model.state_dict(), "best_AFNetResNet_planar_only.pth")
    ...
    torch.save(model.state_dict(), "AFNetResNet_planar_only.pth")
    print("✅ Model saved!")
    tb_writer.close()
